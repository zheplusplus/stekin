...
util::sptr<inst::mediate_base> arithmetics::inst(util::sref<inst::scope const> scope) const
{
    return std::move(mkdirect(new inst::arithmetics(std::move(expr->inst(scope)))));
}

util::sptr<inst::mediate_base> var_def::inst(util::sref<inst::scope const> scope) const
{
...
util::sptr<inst::mediate_base> branch::inst(util::sref<inst::scope const> scope) const
{
...
util::sptr<inst::mediate_base> loop::inst(util::sref<inst::scope const> scope) const
{
    return std::move(util::mkmptr(
                    new loop_mediate(std::move(condition->inst(scope)), body->get_stmts(), scope)));
}

util::sptr<inst::mediate_base> func_ret::inst(util::sref<inst::scope const> scope) const
{
...
util::sptr<inst::mediate_base> func_ret_nothing::inst(util::sref<inst::scope const> scope) const
{
