...
{
    misc::position pos(3);
    inst::Negation(pos, std::move(util::mkptr(new CondTypeExpr)));
    ASSERT_FALSE(error::hasError());
    inst::Negation(pos, std::move(util::mkptr(new BadTypeExpr)));
    ASSERT_FALSE(error::hasError());

    inst::Negation(pos, std::move(util::mkptr(new VoidTypeExpr)));
    ASSERT_TRUE(error::hasError());
    ASSERT_EQ(1, getCondNotBools().size());
