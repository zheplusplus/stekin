...
TEST_F(QueryOperationTest, Arithmetics)
{
                ...
                , [&](std::string const& op)
                  {
                      inst::Operation const* result = inst::Operation::queryBinary(
                              pos, op, inst::Type::BIT_INT, inst::Type::BIT_INT);
    ...
    std::for_each(ARITH_BIN_OP_LIST.begin()
                ...
                , [&](std::string const& op)
                  {
                      inst::Operation const* result = inst::Operation::queryBinary(
                              pos, op, inst::Type::BIT_FLOAT, inst::Type::BIT_FLOAT);
                      ASSERT_FALSE(error::hasError()) << "operation img:" << op << " query failed.";
                      ASSERT_TRUE(op_records.insert(result).second)
                              << "operation img:" << op << " duplicate operation.";
                      ...
                      result = inst::Operation::queryBinary(
                              pos, op, inst::Type::BIT_FLOAT, inst::Type::BIT_INT);
                      ASSERT_FALSE(error::hasError()) << "operation img:" << op << " query failed.";
                      ASSERT_EQ(inst::Type::BIT_FLOAT, result->ret_type);

                      result = inst::Operation::queryBinary(
                              pos, op, inst::Type::BIT_INT, inst::Type::BIT_FLOAT);
                      ASSERT_FALSE(error::hasError()) << "operation img:" << op << " query failed.";
                      ASSERT_EQ(inst::Type::BIT_FLOAT, result->ret_type);
                ...
                , [&](std::string const& op)
                  {
                      ...
                      result = inst::Operation::queryPreUnary(pos, op, inst::Type::BIT_INT);
                      ASSERT_FALSE(error::hasError()) << "operation img:" << op << " query failed.";
                      ASSERT_TRUE(op_records.insert(result).second)
                              << "operation img:" << op << " duplicate operation.";
                      ASSERT_EQ(inst::Type::BIT_INT, result->ret_type);

                      result = inst::Operation::queryPreUnary(pos, op, inst::Type::BIT_FLOAT);
                      ASSERT_FALSE(error::hasError()) << "operation img:" << op << " query failed.";
                      ASSERT_TRUE(op_records.insert(result).second)
                              << "operation img:" << op << " duplicate operation.";
                      ASSERT_EQ(inst::Type::BIT_FLOAT, result->ret_type);
...
TEST_F(QueryOperationTest, Booleans)
{
    ...
    std::set<inst::Operation const*> op_records;
    inst::Operation const* result = inst::Operation::queryBinary(
                pos, "=", inst::Type::BIT_BOOL, inst::Type::BIT_BOOL);
    ...
    ASSERT_EQ(inst::Type::BIT_BOOL, result->ret_type);
    result = inst::Operation::queryBinary(pos, "!=", inst::Type::BIT_BOOL, inst::Type::BIT_BOOL);
    ASSERT_FALSE(error::hasError()) << "operation img:!= query failed.";
    ASSERT_TRUE(op_records.insert(result).second) << "operation img:!= duplicate operation.";
    ASSERT_EQ(inst::Type::BIT_BOOL, result->ret_type);
    ASSERT_FALSE(error::hasError());
                ...
                , [&](std::string const& op)
                  {
                      ...
                      result = inst::Operation::queryBinary(
                              pos, op, inst::Type::BAD_TYPE, inst::Type::BAD_TYPE);
                      ASSERT_FALSE(error::hasError()) << "operation img:" << op << " query failed.";
                      ASSERT_EQ(inst::Type::BAD_TYPE, result->ret_type);

                      result = inst::Operation::queryBinary(
                              pos, op, inst::Type::BAD_TYPE, inst::Type::BIT_INT);
                      ASSERT_FALSE(error::hasError()) << "operation img:" << op << " query failed.";
                      ASSERT_EQ(inst::Type::BAD_TYPE, result->ret_type);

                      result = inst::Operation::queryBinary(
                              pos, op, inst::Type::BIT_FLOAT, inst::Type::BAD_TYPE);
                      ASSERT_FALSE(error::hasError()) << "operation img:" << op << " query failed.";
                      ASSERT_EQ(inst::Type::BAD_TYPE, result->ret_type);

                      result = inst::Operation::queryBinary(
                              pos, op, inst::Type::BAD_TYPE, inst::Type::BIT_BOOL);
                      ASSERT_FALSE(error::hasError()) << "operation img:" << op << " query failed.";
                      ASSERT_EQ(inst::Type::BAD_TYPE, result->ret_type);

                      result = inst::Operation::queryBinary(
                              pos, op, inst::Type::BIT_VOID, inst::Type::BAD_TYPE);
                      ASSERT_FALSE(error::hasError()) << "operation img:" << op << " query failed.";
                      ASSERT_EQ(inst::Type::BAD_TYPE, result->ret_type);
                  });
...
TEST_F(QueryOperationTest, BadOperation)
{
    ...
    ASSERT_EQ("<", getNABinaryOps()[0].op_img);
    ASSERT_EQ(inst::Type::BIT_BOOL->name(), getNABinaryOps()[0].lhst_name);
    ASSERT_EQ(inst::Type::BIT_FLOAT->name(), getNABinaryOps()[0].rhst_name);
    clearErr();
