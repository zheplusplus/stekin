...
TEST(Pointer, SRef)
{
    ...
    int const* ax = &x;
    int const* ay = &y;
    ASSERT_EQ(util::mkref(x), util::mkref(y));
    ASSERT_NE(util::mkref(ax), util::mkref(ay));
    y = 2;
    ASSERT_LT(util::mkref(x), util::mkref(y));
    x = 3;
    ASSERT_LT(util::mkref(y), util::mkref(x));

    ASSERT_TRUE(bool(util::mkref(x)));
    ASSERT_TRUE(bool(util::mkref(y)));
    ASSERT_TRUE(bool(util::mkref(ax)));
    ASSERT_TRUE(bool(util::mkref(ay)));
    ASSERT_FALSE(bool(util::sref<int const>(NULL)));
